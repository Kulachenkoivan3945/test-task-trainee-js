...
//общее количество страниц в таблице, окргуляется до большего целого чтобы не потерять часть записей
const pagesCount = Math.ceil(data.length / rowsCount);
//хранит теекущую страницу
let selectedPage;
//хранит запись, выбранную для изменения
let editedRecord;

//массив для определения какие столбцы скрыты, используется для сокрытия нужных столбцов при добавлении в таблицу записей ("" - стобец скрыт)
let hiddenColumns = ["", 1, 2, 3, 4];

//хранит сортитуемую колонку
let sortedColumn;

//тип сортировки - true, если от а до я
let order;
...
function setPages() {
    ...
    selectedPage = container.children[1];
    addChangePageEventListener();
}

function addChangePageEventListener() {
    document.querySelectorAll('.page-container p').forEach(p => p.addEventListener('click', (() => {
        document.querySelector('.selected-page').classList.remove('selected-page');
        index = Array.from(container.children).indexOf(p);
        tbody.textContent = '';
        //страница, с которой начинается отсчет 
        let firstPageInList;

        let isLastPages = false;
        //номер выбранной страницы
        let pageNumber = Number(p.textContent);
        selectedPage = p;
        switch (index) {
            //переход на первую из любого места
            case 0: {
                pageNumber = 1;
                firstPageInList = 1;
                selectedPage = container.children[1];
            ...
            case 1:
                {
                    //переход на самую левую страницу в списке, т.к список динамический, то в зависимости от номера старницы 
                    //либо список пролистывается на 2 страницы влево, либо на одну,либо остается на месте если страница первая
                    firstPageInList = pageNumber;
                    if (pageNumber > 2) {
                        //если свигается на 2 влево, то выбранная страница становится посередине списка (container.children[3])
                        selectedPage = container.children[3];
                        firstPageInList = pageNumber - 2;
                    }
                    else if (pageNumber > 1) {
                        //если свигается на 2 влево, то выбранная страница становится второй слева (container.children[2])
                        selectedPage = container.children[2];
                        firstPageInList = pageNumber - 1;
                    }
                    else {
                        selectedPage = container.children[1];
                    }
            ...
            case 2:
                {
                    //переход на вторую слева страницу в списке, в зависимости от номера старницы 
                    //либо список пролистывается на 1 страницу влево,либо остается на месте
                    firstPageInList = pageNumber - 1;
            ...
            case 3:
                {
                    //при клике на среднюю страницу - ничего не меняется
                    selectedPage = container.children[3];
                    firstPageInList = pageNumber - 2;
                    break;
                }
            ...
            case 4: {
                //переход на вторую справа страницу в списке, в зависимости от номера старницы 
                //либо список пролистывается на 1 страницу вправо,либо остается на месте
                firstPageInList = pageNumber - 2;
                if (pageNumber >= pagesCount - 1) {
                    selectedPage = container.children[4];
                    firstPageInList = pageNumber - 3;
                }
                else {
                    selectedPage = container.children[3];
                }
            ...
            case 5: {
                //переход на самую правую страницу в списке, в зависимости от номера старницы 
                //либо список пролистывается на 2 страницы вправо, либо на одну,либо остается на месте, если страница последняя
                firstPageInList = pageNumber - 2;
                if (pageNumber == pagesCount) {
                    //если последняя
                    selectedPage = container.children[5];
                    isLastPages = true;
                }
                ...
                else if (pageNumber <= pagesCount - 2) {
                    selectedPage = container.children[3];
                    firstPageInList = pageNumber - 2;
                }
                break;
            ...
            case 6: {
                //переход на последную страницу в таблице
                pageNumber = pagesCount;
                firstPageInList = pagesCount - 4;
                selectedPage = container.children[5];
                break;
            }
        ...
        //заполнение  аблицы строками в зависимости от страницы
        fillTableByPages(pageNumber);
        if (!isLastPages) {
            //сдвиг списка доступных страниц
            for (let i = 1; i <= 5; i++) {
                container.children[i].textContent = firstPageInList;
                firstPageInList++;
            }
        }
    })));
...
//заполнение таблицы всеми записями из массива
function fillAllTable() {
    counter = 0;
    for (const key of data) {
        addRowToTable(counter, key);
        counter++;
    }
    addEditRecordEventListeners();
}

//заполнение таблицы в зависимости от выбранной страницы
//i + page*rowsCount = выбор необходиммого элемента массива в зависимости от страницы (12 элемент = 2+1(на первой странице)*10)
function fillTableByPages(page) {
    page--;
    for (let i = 0; i < rowsCount; i++) {
        if (i + page * rowsCount < data.length) {
            addRowToTable(i + page * rowsCount, data[i + page * rowsCount])
        }
    }
    addEditRecordEventListeners();
}
...
function addRowToTable(id, data) {
    // в зависимости от состояния элемента массива hiddenColumns для каждой ячейки либо присваивается класс hidden, если колонка сокрыта, либо нет
    let row = `<tr id="${id}">`;
    if (hiddenColumns[1] == "") row += `<td class="hidden">${data['name']['firstName']}</td>`;
    else row += `<td>${data['name']['firstName']}</td>`;

    if (hiddenColumns[2] == "") row += ` <td class="hidden">${data['name']['lastName']}</td>`;
    else row += ` <td>${data['name']['lastName']}</td>`;

    if (hiddenColumns[3] == "") row += `<td class="text-clip hidden">${data['about']}</td>`;
    else row += `<td class="text-clip">${data['about']}</td>`;

    if (hiddenColumns[4] == "") row += `
    <td class="color-td hidden">
        <div class="color-box" style="background-color:${data['eyeColor']}"></div>
        <span class="hidden-text">${data['eyeColor']}</span>
    </td> `;
    else row += `
    <td class="color-td">
        <div class="color-box" style="background-color:${data['eyeColor']}"></div>
...
//функция, вызывыаемая при смене состояния ческбокса - отображать по страницам
function changeTableViewMode() {
    //обнуление записей в таблице
    tbody.textContent = "";
    if (!checkbox_view_mode.checked) {
        fillAllTable();
    }
    else {
        fillTableByPages(Number(selectedPage.textContent));
    }
    page_container.classList.toggle('hidden');
    //установка на добавленные элементы таблицы обработчика событий для изменения записей
}


function addEditRecordEventListeners() {
    document.querySelectorAll('tbody tr').forEach(row => row.addEventListener('click', (() => {
        if(editedRecord){
            //цветовое выделение выбранной строки
            editedRecord.classList.remove('edited-row');
            if(editedRecord!==row)editedRecord.classList.remove('selected-row');
        }
        editedRecord = row;
        document.querySelector("#editForm").scrollIntoView({ behavior: 'smooth' });
        //присовение полям ввода значений выбранной строки
        firstName.value = row.cells[0].textContent;
        lastName.value = row.cells[1].textContent;
        about.value = row.cells[2].textContent;
        //lastElementChild т.к в данной ячейке есть div для отображения цвета и скрытый span для буквенного обозначения
        eyeColor.value = row.cells[3].lastElementChild.textContent;
        editedRecord.classList.add('selected-row');
    })));
}

function addHideColumnEventListener() {
    document.querySelectorAll('#column-hide-container input').forEach(input => input.addEventListener('click', (() => {
        column = input.parentNode;
        //получения индекса колонки, которую надо скрыть (т.к в массиве индксы с 0, а в селекторах css с 1),
        // то поскольку первый элемент массива - p с надписью то индекс точно указывает на колонку,скрываемую через css
        index = Array.from(column.parentNode.children).indexOf(column);
        isHidded = input.checked;
        hideColumn(index, isHidded);
    })));
}

//сокрытие колонки
function hideColumn(index, isHidded) {
    //при сокрытии/показе описания добавлаяется/удаляется правая граница для предотвращения наложении двух границ друг на друга
    //(исключительно для красоты отображения) 
    if (index == 2) document.querySelectorAll(`td:nth-child(${index - 1})`).forEach(td => td.classList.toggle('border-rigth-gray'));

    //сокрытие/показ всех ячеек выдранного столбца
    document.querySelector(`th:nth-child(${index})`).classList.toggle('hidden');
    document.querySelectorAll(`td:nth-child(${index})`).forEach(td => td.classList.toggle('hidden'));

    //нужно чтобы столбцы оставались скрытми после смены страницы
    if (isHidded) hiddenColumns[index] = "";
    else hiddenColumns[index] = index;
}

//сохранение изменений в таблице и массиве с данными
function saveChanges() {
    //firstName,lastName,about, eyeColor - id полей ввода на форме
    data[editedRecord.id]['name']['firstName'] = editedRecord.cells[0].textContent = firstName.value;
    data[editedRecord.id]['name']['lastName'] = editedRecord.cells[1].textContent = lastName.value;
    data[editedRecord.id]['about'] = editedRecord.cells[2].textContent = about.value;
    //lastElementChild т.к в данной ячейке есть div(first) для отображения цвета и скрытый span(last) для буквенного обозначения
    data[editedRecord.id]['eyeColor'] = editedRecord.cells[3].lastElementChild.textContent = eyeColor.value;
    editedRecord.cells[3].firstElementChild.style.backgroundColor = eyeColor.value;
    editedRecord.classList.remove('selected-row');
    editedRecord.classList.add('edited-row');
    setTimeout(() => editedRecord.classList.remove('edited-row'), 2000);
    editedRecord.scrollIntoView({ behavior: 'smooth' });
}

//сортировка таблицы по столбцам
function addSortByColumnEventListener() {
    //функция сравнения, принимает тип сортировки (от а до я или обратно) и индекс сортируемого столбца а и b - соседние строки
    //т.к строки сортируются в алфавитном порядке, то используется localCompare()
    const comparer = (order, index) => (a, b) => order ? a.cells[index].textContent.localeCompare(b.cells[index].textContent) :
        b.cells[index].textContent.localeCompare(a.cells[index].textContent);

    document.querySelectorAll('th').forEach(column => column.addEventListener('click', (() => {
        if (column != sortedColumn) order = true;
        else order = !order;
        sortedColumn = column;
        columnIndex = Array.from(column.parentNode.children).indexOf(column);

        if (checkbox_sort_mode.checked && checkbox_view_mode.checked) {
            //если необходимо сортировать по всей таблице в постраничном режиме
            tbody.textContent = "";
            sortAllTable(order, columnIndex);
            fillTableByPages(Number(selectedPage.textContent));
        }
        else {
            rowsArray = Array.from(tbody.children);
            rowsArray.sort(comparer(order, columnIndex)).forEach(row => tbody.appendChild(row));
        }
    })));
}

//функция для сортировки по всей таблице - сортировка исходного массива данных
function sortAllTable(order, index) {
    const comparerAll = (order, index) => (a, b) => {
        switch (index) {
            case 0:
                return order ? a['name']['firstName'].localeCompare(b['name']['firstName']) :
                    b['name']['firstName'].localeCompare(a['name']['firstName']);
            case 1:
                return order ? a['name']['lastName'].localeCompare(b['name']['lastName']) :
                    b['name']['lastName'].localeCompare(a['name']['lastName']);
            case 2:
                return order ? a['about'].localeCompare(b['about']) :
                    b['about'].localeCompare(a['about']);
            case 3:
                return order ? a['eyeColor'].localeCompare(b['eyeColor']) :
                    b['eyeColor'].localeCompare(a['eyeColor']);
        }
    }
    data.sort(comparerAll(order, index));
}